...
class c1 extends Controller
{
    ...
    //* =====================================
    //* ==== Approbation depuis l'email ====
    //* =====================================
    public function approveUserFromEmail(Request $request, User $user)
    {
        if (!$request->hasValidSignature()) {
            abort(401, 'Ce lien d\'approbation n\'est pas valide ou a expiré.');
        }

        $user->update(['is_approved' => true]);

        return redirect()->route('login')
            ->with('success', 'Le compte de ' . $user->name . ' a été approuvé avec succès. Vous pouvez maintenant vous connecter.');
    }
}